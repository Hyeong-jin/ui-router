...
-
**참고:** *UI-Router는 한창 개발 중이다. 따라서, 이 라이브러리는 빈번하게 테스트되는 동안, API가 바뀔 수 있다.
운영 애플리케이션에서의 사용은 변경이력에 따라 사용본의 수정에 익숙할 때만 고려하라.*
...
**(3)** [`ui-sref` 디렉티브](https://github.com/angular-ui/ui-router/wiki/Quick-Reference#ui-sref)를 가진 몇 가지 링크를 추가한 것을 알 수 있을 것이다. 스테이트 전환 관리에 대한 추가에서, 상응하는 스테이트가 URL을 가지고 있다면, 이 디렉티브는 `<a />` 엘리먼트의 `href` 속성을 자동으로 생성한다. 다음으로 몇 개의 템플릿을 추가한다. 이것은 `index.html`의 `ui-view` 안으로 삽입 될 것이다. 기억할 것은 그들이 그들의 `ui-view`를 가지고 있다는 것이다! 이것이 중첩하는 스테이트와 뷰의 요점이다.
...
**(4)** 다음으로, 우리는 몇 가지 하위 템플릿을 추가한다. *하위 템플릿은* 그들의 부모의 상태 템플릿의 `ui-view`에 삽입된다.
...
**(6)** 퀵 스타트 예제를 만져 보라.
>**[중복된 스테이트와 뷰의 퀵 스타트 Plunker로 가기](http://plnkr.co/edit/u18KQc?p=preview)**

**(7)** 이것은 수박 겉핥기일 뿐이다.
>**[더욱 깊이 들어가자!](https://github.com/angular-ui/ui-router/wiki)**


### [여러 개의 명명된 뷰](http://plnkr.co/edit/SDOcGS?p=preview)

또 다른 대단한 특징은 템플릿 당 복수의 `ui-view`를 가질 수 있는 능력이다.

**Pro Tip:** ** *복수의 병렬 뷰는 강력한 특징이라서, 뷰의 중첩으로 인터페이스를 더욱 효과적으로 관리하고, 중첩된 상태와 뷰를 연계할 수 있다.*

**(1)** 위에서 설명한 [설정](#get-started) 지침을 따른다.

**(2)** 하나 이상의  `ui-view`를 앱에 추가하고, 이름을 짓는다.
...
[기존의 이슈 찾기](https://github.com/angular-ui/ui-router/search?q=wat%3F&type=Issues)에서 리포팅된 비슷한 이슈를 찾을 수 있습니다.

없다면, 문제점을 재현할 수 있는 [plunkr 생성](http://plnkr.co/edit/u18KQc?p=preview)을 합니다(가능하면 적은 양의 코드로 작성하세요. 적으면 적을 수록 더욱 빨리 문제를 해결할 수 있습니다.)

다음으로, 문제를 간략하게 설명하는 [새로운 이슈를 생성](https://github.com/angular-ui/ui-router/issues/new)하고,
문제를 발생시키는 상황 같은 백그라운드 정보를 약간 제공합니다. 잊지 말고 생성한 plunkr의 링크를 포함합니다.

**Note**: 만약 확신이 없는 기능이 사용되거나, 뜻 밖의 확신하지 않은 행동은 버그입니다.
이러한 것에 대해 리포팅하기 전에 물어볼 수 있는 가장 좋은 곳은
[Google Group](https://groups.google.com/forum/#!categories/angular-ui/router) 또는
...
**(2)** [roadmap](https://github.com/angular-ui/ui-router/issues/milestones)를 확인하여 프로젝트의 방향을 보고, 기능에 대한 아이디어가 프로젝트의 방향과 적합한지 확인한다.

**(3)** 만약 당신이 확실하지 않다면, 당신의 아이디어에 대한 피드백을 얻기 위해서는 [RFC](https://github.com/angular-ui/ui-router/issues/new?title=RFC:%20My%20idea)를 열어보라.

**(4)** 마지막으로, 코드를 커밋하고 pull request를 오픈하라. 다음의 규칙을 따르면 코드와 커밋은 유지될 것이다:

- *항상* 새로운 특징을 위한 테스트 커버리지(나 버그 수정을 위한 역행 테스트)를 갖고, 기존의 테스트를 *절대* 깨뜨리지 않는다
- 커밋은 각각 하나의 논리적인 변경이어 한다. 만약 어떤 기능이 반복적으로 여러 곳을 수정해야 하는 것이면, 커밋을 분리하여 하나씩 커밋한다.
...
* `grunt`: 일반적인 빌드를 수행하며, jshint와 karma 테스트를 실행한다.
